...
message("Loading the raw data tables...")
...
map(names(to_download), function(basename) {
  ...
  cat("Base:", basename, "\n\n")
  names(tables) <- tables <- to_download[[basename]][
    !to_download[[basename]] %in% exclude
  ]
...
message("Creating the production database tables...")
db <- names(to_download) |>
  set_names() |>
  map(function(basename) {
    ...
    map(select_tables, function(tab_name) {
      ...
      select_vars <- tjet[[basename]]$metadata |>
        filter(
          incl_prod == 1 &
            ...
            # incl_data != "transform: multiple" & ### removed because we actually use some multi-select fields now
            ### include these later if creating dummies
            table_name == tab_name
        ) |>
        filter(
          ## need this because some multi-selects are based on another table
      ...
      first <-
        c(
          select_vars[str_detect(select_vars, fixed("ID"))],
...
message("Removing invalid mechanism records...")
...
dim_drop <- map(db, function(dat) {
  ## for keeping track of how many records left
  map_vec(dat, nrow)
})
message("MegaBase:")
cbind(orig = dim_orig[[1]], drop = dim_drop[[1]])
message("Prosecutions:")
cbind(orig = dim_orig[[2]], drop = dim_drop[[2]])

message("Transforming checkbox fields to binary variables...")
  ...
  map(function(basename) {
    ...
    map(select, function(tab_name) {
      ...
      db[[basename]][[tab_name]][, fields] <-
        map(db[[basename]][[tab_name]][, fields], \(col) {
...
message("\n\nProcessing multiple lookup and multiple selection fields...") ### CHECK
db <- names(to_download) |>
  set_names() |>
  map(function(basename) {
...
### processing tables for multiselect fields in the MegaBase
...
### list columns due to multiple selection fields; need to deal with these below

# tjet[["Prosecutions"]][["metadata"]] |>
#     filter(incl_prod == 1 & incl_data == "transform: multiple") |>
...
db[["Prosecutions"]][["Accused"]] <-
  ...
  rowwise() |>
  mutate(
    chargesCategories = str_flatten(chargesCategories, "; "),
    genderAttentiveCharges = str_flatten(genderAttentiveCharges, "; "),
    genderVictims = str_flatten(genderVictims, "; ")
  ) |>
  ungroup()

### checking numbers of records again to ensure integrity
message("How many records after processing multi-select fields?")
...
if (
  !length(mega$drop == mega$now) == sum(length(mega$drop == mega$now)) |
    !length(pros$drop == pros$now) == sum(length(pros$drop == pros$now))
) {
  stop(
    "Unexpected change in number of mechanism record! This needs to be resolved for the pipeline to be completed."
  )
}
...
message("Processing conflictIDs and dyadIDs...")
...
### this no longer works vor vettings and trials because the relationships are now one-to-many

# db[["Prosecutions"]][["Trials"]] <-
#   db[["Prosecutions"]][["Trials"]] |>
#   unnest_longer(
#     all_of(c("ucdpConflictID", "ucdpDyadID")),
#     keep_empty = TRUE
#   ) |>
#   rename(airtable_record_id = "ucdpConflictID") |>
#   left_join(
#     tjet[["Prosecutions"]]$Conflicts |>
#       select(airtable_record_id, conflict_id),
#     by = "airtable_record_id"
#   ) |>
#   select(-airtable_record_id) |>
...
#   left_join(
#     tjet[["Prosecutions"]]$Dyads |>
#       select(airtable_record_id, dyad_id),
#     by = "airtable_record_id"
#   ) |>
#   select(-airtable_record_id) |>
#   rename(
#     ucdpConflictID = "conflict_id",
#     ucdpDyadID = "dyad_id"
#   ) |>
#   group_by(trialID) |>
...
db[["MegaBase"]][["Reparations_Conflicts"]] <-
  db[["MegaBase"]][["Reparations"]] |>
  select(reparationID, ucdpConflictID) |>
...
db[["Prosecutions"]][["Trials_Conflicts"]] <-
  db[["Prosecutions"]][["Trials"]] |>
  select(trialID, ucdpConflictID) |>
  unnest_longer(ucdpConflictID) |>
  rename(airtable_record_id = "ucdpConflictID") |>
  left_join(
    tjet[["Prosecutions"]]$Conflicts |>
...
db[["Prosecutions"]][["Trials_Dyads"]] <-
  db[["Prosecutions"]][["Trials"]] |>
  select(trialID, ucdpDyadID) |>
  unnest_longer(ucdpDyadID) |>
  rename(airtable_record_id = "ucdpDyadID") |>
  left_join(
    tjet[["Prosecutions"]]$Dyads |>
...
db[["Prosecutions"]][["Trials"]] <-
  db[["Prosecutions"]][["Trials"]] |>
  mutate(
    # yearEnd = ifelse(is.na(yearEnd) & ongoing == 1, 2024, yearEnd),
    # yearEnd = ifelse(
    #   is.na(yearEnd) & ongoing == 0 & CLs_final_year > 1970,
    #   CLs_final_year,
    #   yearEnd
    # ),
    # yearEnd = ifelse(is.na(yearEnd) & ongoing == 0, yearStart, yearEnd),
    yearEnd = ifelse(
      is.na(yearEnd) & year_last >= yearStart,
      year_last,
      yearEnd
    ),
    yearEnd = ifelse(
      is.na(yearEnd) & is.na(year_last) & yearStart <= 2020,
      yearStart,
      yearEnd
    ),
    yearEnd = ifelse(
      is.na(yearEnd) & is.na(year_last) & yearStart > 2020,
      ### this is temporary, until we code 2021-2023 systematically,
      ### currently this only affects a few cases that don't make it into the public dataset
      ### but leaving these as missing would break the pipeline
      2025,
      yearEnd
    ),
    caseDescription = str_squish(caseDescription),
    charnum = nchar(caseDescription),
    caseDescription = ifelse(
      str_sub(caseDescription, charnum, charnum) == ".",
...
fair_trials <- db[["MegaBase"]][["Transitions"]] |>
  ...
  mutate(
    year = ifelse(before == 1, 1970, year),
...
message("Processing transitions data... ")
db[["MegaBase"]][["Transitions"]] <-
  db[["MegaBase"]][["Transitions"]] |>
  filter(trans == 1) |>
  mutate(
    p5 = case_when(
      is.na(p5_year) | trans_year_begin < p5_year ~ 0,
      trans_year_begin >= p5_year ~ 1
    ),
    bmr = case_when(
      is.na(bmr_year) | trans_year_begin < bmr_year ~ 0,
      trans_year_begin >= bmr_year ~ 1
    ),
    ert = case_when(
      is.na(ert_year) | trans_year_begin < ert_year ~ 0,
  ...
  mutate(
    sources = str_flatten(
      c(
        ...
        case_when(
          !is.na(ert_year) ~ paste("VDem-ERT (", ert_year, ")", sep = "")
        )
      ),
      collapse = " & ",
      na.rm = TRUE
...
message("Filtering mechanisms records to meet TJET criteria for inclusion... ")
...
db[["Prosecutions"]][["Trials"]] <-
  ...
  # filter(IntraConfl == 1 | humanRights == 1 | HRs_charges > 0) ### not longer needed after recoding
  filter(IntraConfl == 1 | HRVorCoreCrimes == 1)
...
chk_n <- nrow(db[["Prosecutions"]][["Accused"]])
...
if (nrow(db[["Prosecutions"]][["Accused"]]) != chk_n) {
  stop("need to fix multiple individuals per accused in Airtable")
}
...
trials_to_include <- db[["Prosecutions"]][["Accused"]] |>
  ...
  filter(
    stateRelation != "neither state nor opposition member"
    # & (str_detect(stateRelation, "state agent") | str_detect(stateRelation, "opposition member"))
  ) |>
  select(trialID) |>
  distinct() |>
  arrange(trialID) |>
...
db[["MegaBase"]][["TruthCommissions"]] <-
  db[["MegaBase"]][["TruthCommissions"]] |>
  filter(
    temporaryBodyReport == 1 & # authorizedByState == 1 &
      focusedPast == 1 &
      investigatePatternAbuse == 1 &
      neverOperated == 0
  ) |>
  select(
    -temporaryBodyReport,
    -focusedPast, # -authorizedByState,
    -investigatePatternAbuse,
    -neverOperated
  )
...
### UN investigations
message("Processing UN investigations data... ")
dl_invest <- db[["MegaBase"]][["Investigations"]] |>
  ...
  mutate(
    ...
    mandate = ifelse(
      is.na(mandate) & ccode_cow == 93 & unsc == 1,
      "UNSC",
      mandate
    ),
    mandate = ifelse(
      mandate == "rep" & ccode_cow == 860 & secgen == 1,
      "SECGEN",
      mandate
    ),
    ...
    goals = case_when(
      uninv_intlpros == 1 &
        uninv_evcoll == 1 &
        uninv_dompros ==
          1 ~ "support international prosecutions; collect evidence; encourage domestic prosecutions",
      uninv_intlpros == 1 &
        uninv_evcoll ==
          1 ~ "support international prosecutions; collect evidence",
      uninv_intlpros == 1 &
        uninv_dompros ==
          1 ~ "support international prosecutions; encourage domestic prosecutions",
      uninv_evcoll == 1 &
        uninv_dompros ==
          1 ~ "collect evidence; encourage domestic prosecutions",
...
db[["MegaBase"]][["Investigations"]] <- dl_invest |>
  ...
  left_join(
    dl_invest |>
      ...
      reframe(
        uninv_dompros_beg = max(uninv_dompros),
...
tj_laws <- db[["MegaBase"]][["Legal"]] |>
  filter(!is.na(type) & type %in% c("decree", "law", "peace agreement")) |>
  select(legalID, ccode, year) |>
  reframe(
    .by = c(ccode, year),
    tj_laws = n()
  ) |>
...
message("Preparing data for TJET country-year dataset... ")

### creating country list as basis for country-year dataset
countrylist <- db[["Countries"]] |>
  mutate(
    beg = as.integer(str_sub(begin_date, 1, 4)),
...
countrylist <- countrylist |>
  left_join(
    countrylist |>
      ...
      filter(
        !country %in% c("Soviet Union", "Yugoslavia", "Serbia and Montenegro")
      ) |>
      rename("country_case" = "country", "country_case_fr" = "country_fr"),
...
translist <- read_csv(
  "transitions/transitions_new_revised.csv",
  show_col_types = FALSE
) |>
  full_join(
    countrylist |>
  ...
  mutate(
    transition = ifelse(is.na(trans_year), 0, 1),
    dem_polity = ifelse(polity_p5 >= 6, 1, 0),
    dem_vdem = ifelse(
      str_detect(str_to_lower(v2x_regime_amb), "democracy"),
      1,
      0
    ),
    dem_all = rowSums(
  ...
  ungroup() |>
  mutate(
    context_bmr = case_when(
      dem_bmr_min == 0 & dem_bmr_max == 0 ~ 0,
      dem_bmr_min == 1 & dem_bmr_max == 1 ~ 1
    ),
    context_polity = case_when(
      dem_polity_min == 0 & dem_polity_max == 0 ~ 0,
      dem_polity_min == 1 & dem_polity_max == 1 ~ 1
    ),
    context_vdem = case_when(
      dem_vdem_min == 0 & dem_vdem_max == 0 ~ 0,
...
message("Creating country-year dataset with mechanisms count measures... ")
...
db[["CountryYears"]] <- map(countrylist$country, function(ctry) {
  ...
  mutate(
    trials_domestic = ifelse(is.na(trials_domestic), 0, trials_domestic),
    trials_domestic_SGBV = ifelse(
      is.na(trials_domestic_SGBV),
      0,
      trials_domestic_SGBV
    )
  ) |>
  left_join(intl, by = c("ccode", "year")) |>
  mutate(
    trials_intl = ifelse(is.na(trials_intl), 0, trials_intl),
    trials_intl_SGBV = ifelse(is.na(trials_intl_SGBV), 0, trials_intl_SGBV)
  ) |>
  left_join(foreign, by = c("ccode", "year")) |>
  mutate(
    trials_foreign = ifelse(is.na(trials_foreign), 0, trials_foreign),
    trials_foreign_SGBV = ifelse(
      is.na(trials_foreign_SGBV),
      0,
      trials_foreign_SGBV
    )
  )

message("Total counts of TJ mechanims in country-year datatset:")
db[["CountryYears"]] |>
  reframe(
...
message("Creating other production database tables... ")
...
db[["codebook"]] <- read_csv(
  here::here("data", "tjet_codebook.csv"),
  show_col_types = FALSE,
  guess_max = 2000
) |>
  tibble()
...
db[["fields_meta"]] <- read_csv(
  here::here("data", "tjet_fields_meta.csv"),
  show_col_types = FALSE
) |>
  tibble()
...
db[["translations"]] <- read_csv(
  here::here("data", "tjet_translations.csv"),
  show_col_types = FALSE
) |>
  tibble()
...
db[["ConflictDyads"]] <- read_csv(
  here::here("conflicts", "confl_dyads.csv"),
  show_col_types = FALSE
) |>
  tibble() |>
...
### database version
db[["db_timestamp"]] <- tjet[["db_timestamp"]] |>
  tibble(tjet_timestamp = _)
...
gloss <- read_csv(
  here::here("data", "HHI_surveys_glossary.csv"),
  show_col_types = FALSE
) |>
  rename(
    from = "Appears as",
...
map(db[["SurveysMeta"]]$results_tables, function(filename) {
  ...
  surveytab <- readxl::read_xlsx(
    here::here("data", "downloads", filename),
    .name_repair = "unique_quiet"
  )
  tooltips <- names(surveytab)
  tooltips[str_detect(tooltips, fixed("..."))] <- NA
  if (is.na(tooltips[1]) & surveytab[1, 1] == "Section") {
    tooltips[1] <- "Section"
  }
  if (is.na(tooltips[2]) & surveytab[1, 2] == "Question") {
    tooltips[2] <- "Question"
  }
  if (is.na(tooltips[3]) & surveytab[1, 3] == "Responses") {
    tooltips[3] <- "Responses"
  }
  tooltips <- fillr::fill_missing_previous(tooltips)
  tooltips[is.na(tooltips)] <- ""
  if (is.na(surveytab[1, 1]) & tooltips[1] == "Section") {
    surveytab[1, 1] <- "Section"
  }
  if (is.na(surveytab[1, 2]) & tooltips[2] == "Question") {
    surveytab[1, 2] <- "Question"
  }
  if (is.na(surveytab[1, 3]) & tooltips[3] == "Responses") {
    surveytab[1, 3] <- "Responses"
  }
  ...
  surveytab <- surveytab |>
    mutate(
      Question = str_replace_all(Question, gl),
...
message("Processing multi-select tables for mechanisms... ")
  ...
  map(function(tabname) {
    ...
    temp <- map(multitabs, function(multitab) {
      if ("labelID" %in% names(db[[multitab]])) {
        ...
        var <- names(db[[multitab]])[
          !names(db[[multitab]]) %in% tabs[[tabname]]
        ]
        db[[multitab]] |>
          group_by(across(all_of(tabs[[tabname]]))) |>
          mutate(across(all_of(var), function(x) {
...
### vettings
message("Processing vettings data... ")
db[["Vettings"]] <- db[["Vettings"]] |>
  mutate(
    policy_type = type_dismissal + type_ban + type_declassification,
    inst_exe = case_when(
    ...
    fairness = ifelse(
      str_detect(targetingWhy, "specific individual conduct") |
        appealJudgment == "yes",
      1,
      0
    ) +
      ...
      ifelse(
        courtChallenge != "no court challenge" &
          courtChallenge != "don't know" &
          !is.na(courtChallenge),
        1,
        0
      )
  )
...
db[["ICCaccused"]] <- db[["CaseEvents"]] |>
  filter(str_detect(outcome, "ICC: ")) |>
  filter(
    outcome %in%
      c(
        "ICC: arrest warrant issued",
        "ICC: arrest and appearance at court",
        "ICC: confirmation of charges",
        "ICC: proceedings began",
        "ICC: charges withdrawn or case dismissed"
      )
  ) |>
  mutate(
    outcome = case_when(
  ...
  pivot_wider(names_from = outcome, values_from = year) |>
  left_join(
    db[["Accused"]] |>
      select(accusedID, trialID, name, position_desc, position_desc_fr),
    by = "accusedID"
  ) |>
  left_join(
    db[["Trials"]] |>
...
message("Loading covariates for dataset... ")
...
df <- df |>
  ...
  mutate(
    country = ifelse(country == "Ethiopia, FDR", "Ethiopia", country),
    country_case = ifelse(
      country_case == "Ethiopia, FDR",
      "Ethiopia",
      country_case
    )
  ) |>
...
message("Assembling dataset for website and analyses... ")
...
df <- df |>
  left_join(
    ...
    by = c("ccode_cow", "ccode_ksg", "country_id_vdem", "year")
  ) |> # losing Slovenia 1991?
  ...
  mutate(
    amnesties_sample = ifelse(amnesties > 0, 1, NA),
  ...
  mutate(
    amnesties_sample = ifelse(is.na(amnesties_sample), 0, amnesties_sample),
    reparations_sample = ifelse(
      is.na(reparations_sample),
      0,
      reparations_sample
    ),
    tcs_sample = ifelse(is.na(tcs_sample), 0, tcs_sample),
    trials_domestic_sample = ifelse(
      is.na(trials_domestic_sample),
      0,
      trials_domestic_sample
    ),
  ...
  ungroup() |>
  left_join(
    read_csv("data/icc_statesparty.csv", show_col_types = FALSE) |>
      filter(ccode != 511) |>
  ...
  mutate(
    icc_sp = ifelse(
      is.na(icc_sp) & country == "Andorra" & year >= 2001,
      1,
      icc_sp
    ),
    ...
    icc_sp = ifelse(
      is.na(icc_sp) & country == "Croatia" & year >= 2001,
      1,
      icc_sp
    ),
    icc_sp = ifelse(
      is.na(icc_sp) & country == "Dominica" & year >= 2001,
      1,
      icc_sp
    ),
    icc_sp = ifelse(
      is.na(icc_sp) & country == "Grenada" & year >= 2011,
      1,
      icc_sp
    ),
    icc_sp = ifelse(
      is.na(icc_sp) & country == "Kiribati" & year >= 2019,
      1,
      icc_sp
    ),
    icc_sp = ifelse(
      is.na(icc_sp) & country == "Liechtenstein" & year >= 2001,
      1,
      icc_sp
    ),
    icc_sp = ifelse(
      is.na(icc_sp) & country == "Marshall Islands" & year >= 2000,
      1,
      icc_sp
    ),
    ...
    icc_sp = ifelse(
      is.na(icc_sp) & country == "Nauru" & year >= 2001,
      1,
      icc_sp
    ),
    icc_sp = ifelse(is.na(icc_sp) & country == "Oman", 0, icc_sp),
    icc_sp = ifelse(
      is.na(icc_sp) & country == "Saint Lucia" & year >= 2010,
      1,
      icc_sp
    ),
    icc_sp = ifelse(
      is.na(icc_sp) & country == "Samoa" & year >= 2002,
      1,
      icc_sp
    ),
    icc_sp = ifelse(
      is.na(icc_sp) & country == "San Marino" & year >= 1999,
      1,
      icc_sp
    ),
    ...
    icc_sp = ifelse(
      is.na(icc_sp) & country == "Vanuatu" & year >= 2011,
      1,
      icc_sp
    ),
    icc_sp = ifelse(
      is.na(icc_sp) & country == "Burundi" & year >= 2017,
      0,
      icc_sp
    ),
    icc_sp = ifelse(
      is.na(icc_sp) & country == "Philippines" & year >= 2019,
      0,
      icc_sp
    ),
  ...
  mutate(
    sample_trans = ifelse(transition == 1, year, NA),
    sample_confl_25 = ifelse(
      confl_new_25 == 1 | confl_recur_25 == 1 | confl_cont_25 == 1,
      year,
      NA
    ),
    sample_confl_100 = ifelse(
      confl_new_100 == 1 | confl_recur_100 == 1 | confl_cont_100 == 1,
      year,
      NA
    ),
    sample_confl_1000 = ifelse(
      confl_new_1000 == 1 | confl_recur_1000 == 1 | confl_cont_1000 == 1,
      year,
      NA
    ),
    ...
    dco_1000 = ifelse(
      !is.na(sample_confl_1000) & year == sample_confl_1000,
      1,
      0
    ),
    # dco = dco_25
  ...
  group_by(country_case) |>
  mutate(
    sample_trans = min_inf_to_na(sample_trans),
    ...
    peace_spell_25 = ifelse(
      dco_25 == 0 & (lag(dco_25) == 1 | is.na(lag(dco_25))),
      paste(ccode_ksg, year, sep = "-"),
      NA
    ),
    peace_spell_100 = ifelse(
      dco_100 == 0 & (lag(dco_100) == 1 | is.na(lag(dco_100))),
      paste(ccode_ksg, year, sep = "-"),
      NA
    ),
    peace_spell_1000 = ifelse(
      dco_1000 == 0 & (lag(dco_1000) == 1 | is.na(lag(dco_1000))),
      paste(ccode_ksg, year, sep = "-"),
      NA
    )
  ) |>
  ...
  ungroup() |>
  mutate(
    peace_spell_25 = ifelse(confl_cont_25 > 0, NA, peace_spell_25),
    ...
    sample_confl_100 = ifelse(
      is.infinite(sample_confl_100),
      NA,
      sample_confl_100
    ),
    sample_confl_1000 = ifelse(
      is.infinite(sample_confl_1000),
      NA,
      sample_confl_1000
    ),
    aco_25 = case_when(
      is.na(sample_confl_25) | year < sample_confl_25 ~ 0,
      year >= sample_confl_25 ~ 1
    ),
    aco_100 = case_when(
      is.na(sample_confl_100) | year < sample_confl_100 ~ 0,
      year >= sample_confl_100 ~ 1
    ),
    aco_1000 = case_when(
      is.na(sample_confl_1000) | year < sample_confl_1000 ~ 0,
      year >= sample_confl_1000 ~ 1
    ),
    # aco = aco_25,
    sample_trans = ifelse(is.infinite(sample_trans), NA, sample_trans),
    dtr = case_when(
      is.na(sample_trans) | year < sample_trans ~ 0,
      year >= sample_trans ~ 1
    ),
  ) |>
  left_join(
  ...
  mutate(
    reg_chg = case_when(
      !is.na(dem_reversion) | transition == 1 ~ 1,
      TRUE ~ 0
    ),
    reg_anoc = case_when(
      v2x_regime_ert %in% c("Closed autocracy", "Liberal democracy") ~ 0,
  ...
  mutate(
    fair_postautocratic_trials = ifelse(
      is.na(fair_postautocratic_trials),
      0,
      fair_postautocratic_trials
    )
  ) |>
  ungroup() |>
  arrange(country_case, year) |>
  ...
  mutate(
    cum_theta_mean_fariss = ifelse(isna, NA, cummean(theta_mean_fariss)),
...
message("Creating TJ measures in TJET country-year dataset...")
...
message("Creating TJ year zero variable... ")
df <- df |>
  arrange(country_case, year) |>
  mutate(
    tj_yr_zero = ifelse(
...
message("These variables are in the codebook but not in the dataset:")
...
dropbox_path <- "~/Library/CloudStorage/Dropbox/TJLab/TimoDataWork/analyses_datasets"
...
message("Creating conflict analyses dataset... ")
source(here::here("pipeline", "go", "conflict_peace_spells.R"))
...
db[["Amnesties"]] <- db[["Amnesties"]] |>
  mutate(
    mechanismDescription_fr = str_replace_all(
      mechanismDescription_fr,
      "droits de l'homme",
...
db[["Reparations"]] <- db[["Reparations"]] |>
  mutate(
    officialName_en_fr = str_replace_all(
      officialName_en_fr,
      "droits de l'homme",
      "droits de la personne"
    )
...
db[["Trials"]] <- db[["Trials"]] |>
  mutate(
    caseDescription_fr = str_replace_all(
      caseDescription_fr,
      "droits de l'homme",
      "droits de la personne"
    )
...
db[["Accused"]] <- db[["Accused"]] |>
  mutate(
    position_desc_fr = str_replace_all(
      position_desc_fr,
      "droits de l'homme",
      "droits de la personne"
    )
...
message("Creating data files for download from the TJET website... ")
...
codebook |>
  ...
  mutate(tjet_version = timestamp) |>
  write_csv(
    here::here("tjet_datasets", "tjet_codebook_analyses.csv"),
    na = ""
  ) |>
...
db[["dl_tjet_codebook"]] <- codebook |>
  ...
  rename(source = source_abr) |>
  left_join(
    read_csv(here::here("data", "tjet_sources.csv"), show_col_types = FALSE),
    by = "source"
  ) |>
  mutate(tjet_version = timestamp) |>
...
vars <- db[["dl_tjet_codebook"]]$col_name[
  db[["dl_tjet_codebook"]]$col_name %in% names(df)
]
...
db[["AmnestiesCodebook"]] <- db[["codebook"]] |>
  ...
  write_csv(
    here::here("tjet_datasets", "tjet_amnesties_codebook.csv"),
    na = ""
  ) |>
...
db[["ReparationsCodebook"]] <- db[["codebook"]] |>
  ...
  # arrange(str_to_lower(col_name)) |>
  write_csv(
    here::here("tjet_datasets", "tjet_reparations_codebook.csv"),
    na = ""
  ) |>
...
db[["AccusedCodebook"]] <- db[["codebook"]] |>
  ...
  # arrange(str_to_lower(col_name)) |>
  write_csv(
    here::here("tjet_datasets", "tjet_accused_codebook.csv"),
    na = ""
  ) |>
...
db[["VettingsCodebook"]] <- db[["codebook"]] |>
  ...
  # arrange(str_to_lower(col_name)) |>
  write_csv(
    here::here("tjet_datasets", "tjet_vettings_codebook.csv"),
    na = ""
  ) |>
...
db[["InvestigationsCodebook"]] <- db[["codebook"]] |>
  ...
  # arrange(str_to_lower(col_name)) |>
  write_csv(
    here::here("tjet_datasets", "tjet_un_investigations_codebook.csv"),
    na = ""
  ) |>
  write_csv(
    here::here(dropbox_path, "tjet_un_investigations_codebook.csv"),
    na = ""
  )
db[["Investigations"]] <- dl_invest |>
  ...
  select(
    all_of(
      db[["InvestigationsCodebook"]] |>
        select(col_name) |>
        unlist(use.names = FALSE)
    )
  ) |>
  write_csv(
    here::here("tjet_datasets", "tjet_un_investigations.csv"),
    na = ""
  ) |>
...
db[["ICCcodebook"]] <- db[["codebook"]] |>
  ...
  # arrange(str_to_lower(col_name)) |>
  write_csv(
    here::here("tjet_datasets", "tjet_icc_interventions_codebook.csv"),
    na = ""
  ) |>
  write_csv(
    here::here(dropbox_path, "tjet_icc_interventions_codebook.csv"),
    na = ""
  )
db[["ICC"]] <- db[["ICC"]] |>
  ...
  write_csv(
    here::here("tjet_datasets", "tjet_icc_interventions.csv"),
    na = ""
  ) |>
...
db[["ICCaccusedCodebook"]] <- db[["codebook"]] |>
  ...
  # arrange(str_to_lower(col_name)) |>
  write_csv(
    here::here("tjet_datasets", "tjet_icc_accused_codebook.csv"),
    na = ""
  ) |>
...
message("Creating automatic descriptive texts for website country pages...")
