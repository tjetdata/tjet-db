  ...
  map(select_tables, function(tab_name) { # tab_name = "Amnesties"
    select_vars <- tjet[[basename]]$metadata |>
      ...
      filter(
        ...
        !(incl_data == "transform: multiple" & field_type == "multipleRecordLinks")
      ) |> 
      arrange(order)  |>
      select(field_name) |>
      unlist(use.names = FALSE)
...
db[["Prosecutions"]][["Accused"]] <- db[["Prosecutions"]][["Accused"]] |> 
  rowwise() |> 
  mutate(
    prelimProcess = str_flatten(prelimProcess, "; "), 
...
db[["Prosecutions"]][["Accused"]] <- 
  ...
  unnest_longer(
    all_of(
      c("lastGuiltyYear", "lastVerdictYear", "lastVerdict", 
      "lastSentencingTime", "lastSentencingArrangement")
    ), 
    keep_empty = TRUE
  ) |>
  distinct() 
...
db[["Prosecutions"]][["Trials"]] <- 
  ...
  mutate(
    ...
    caseDescription = str_squish(caseDescription), charnum = nchar(caseDescription),
...
db[["MegaBase"]][["Transitions"]] <-
  ...
  mutate(
    p5 = case_when(
      is.na(p5_year) | trans_year_begin < p5_year ~ 0, trans_year_begin >= p5_year ~ 1
    ),
    bmr = case_when(
      is.na(bmr_year) | trans_year_begin < bmr_year ~ 0, trans_year_begin >= bmr_year ~ 1
    ),
    ert = case_when(
      is.na(ert_year) | trans_year_begin < ert_year ~ 0, trans_year_begin >= ert_year ~ 1
    ), 
    nsupport = p5 + bmr + ert
  ) |> 
  rowwise() |> 
...
trials_to_include <- db[["Prosecutions"]][["Accused"]] |> 
  ...
  filter(
    stateRelation != "neither state nor opposition member" # & (str_detect(stateRelation, "state agent") | str_detect(stateRelation, "opposition member")) 
  ) |> 
  select(trialID) |> 
  distinct() |> 
  arrange(trialID) |> 
...
db[["Prosecutions"]][["Trials"]] <- 
  db[["Prosecutions"]][["Trials"]] |> 
  filter(
    (trialType %in% c("domestic", "don't know") & trialID %in% trials_to_include) | 
...
db[["MegaBase"]][["TruthCommissions"]] <-
  ...
  select(
    -temporaryBodyReport, -focusedPast, # -authorizedByState, 
...
dl_invest <- db[["MegaBase"]][["Investigations"]] |>
  ...
  select(
    ccode_cow, beg, end, name, mandate_en, mandate_fr, uninv_dompros, uninv_intlpros, 
    uninv_evcoll, secgen, unsc, cohr, unga, ohchr, hrc, source
  ) |> 
  ...
  mutate(
    ...
    goals = case_when(
      uninv_intlpros == 1 & uninv_evcoll == 1 & uninv_dompros == 1 ~ "support international prosecutions; collect evidence; encourage domestic prosecutions", 
  ...
  select(
    country, ccode_cow, beg, end, mandate, mandate_en, mandate_fr, 
    goals, uninv_dompros, uninv_evcoll, uninv_intlpros, source
  ) 
...
db[["MegaBase"]][["Investigations"]] <- dl_invest |>
  ...
  select(
    ccode_cow, year, uninv_beg, uninv, uninv_dompros_beg, uninv_dompros, 
...
db <- c(
  db[["MegaBase"]], 
  db[["Prosecutions"]][c(
    "Trials", "Accused", "CourtLevels", "Trials_Crimes", "Trials_Victims", 
...
rm(
  exclude, select, dim_drop, dim_last, dim_now, dim_orig, 
...
countrylist <- db[["Countries"]] |> 
  ...
  select(
    country, country_fr, include, ccode, ccode_case, ccode_ksg, m49, isoa3, country_id_vdem, 
    beg, end, micro_ksg, region, region_sub_un, region_wb, focus, factsheet, 
    starts_with("txt_"), starts_with("auto_")
  ) |> 
...
countrylist <- countrylist |>
  left_join(
    countrylist |> 
      ...
      filter(!country %in% c("Soviet Union", "Yugoslavia", "Serbia and Montenegro") ) |>
      rename("country_case" = "country", "country_case_fr" = "country_fr"),
    by = c("ccode_case" = "ccode")
  ) |> 
  select(
    country, country_case, include, country_fr, country_case_fr, ccode, ccode_case, 
    ccode_ksg, m49, isoa3, country_id_vdem, beg, end, micro_ksg, region, region_sub_un, 
    region_wb, tjet_focus, factsheet, starts_with("txt_"), starts_with("auto_")
  ) 
...
tabs <- c("Amnesties", "Reparations", "TruthCommissions", "Vettings", "Trials", "Accused") 
...
translist <- read_csv("transitions/transitions_new_revised.csv", show_col_types = FALSE) |> 
  full_join(
    countrylist |> 
  ...
  mutate(
    ...
    dem_all = rowSums(across(all_of(c("dem_bmr", "dem_polity", "dem_vdem"))), na.rm = TRUE)/3) |> 
  ...
  select(
    country, ccode, year, transition, dem_bmr, dem_polity, dem_vdem, 
    regime_sample, reg_democ, reg_autoc, reg_trans
  ) |> 
  filter(year <= 2024) 

### democratic reversions data for merging 
reversions <- read_csv("transitions/transitions_new_revised.csv", show_col_types = FALSE) |> 
  select(
    country, country_id_vdem, year, reg_type, reg_start_year, reg_end_year, 
...
db[["CountryYears"]] <- map(countrylist$country , function(ctry) {
  ...
  select(
    cyID, country, country_fr, country_label, country_label_fr, year, beg, end, ccode, 
    ccode_case, country_case, ccode_ksg, country_id_vdem, tjet_focus, region, regime_sample, 
    reg_democ, reg_autoc, reg_trans, conflict, transition
  ) |> 
...
db[["Countries"]] <- countrylist |> 
  ...
  select(
    country, country_case, include, country_fr, ccode, ccode_case, 
...
db[["SurveysMeta"]] <- db[["SurveysMeta"]] |>
  ...
  select(
    country, country_fr, year, date_start, date_end, section_title, 
  ...
  temp <- map(multitabs, function(multitab) { 
    if("labelID" %in% names(db[[multitab]])) {
      db[[multitab]] |> 
        ...
        mutate(
          !!str_replace(multitab, paste(tabname, "_", sep = ""), "") := 
...
db[["Vettings"]] <- db[["Vettings"]] |>
  mutate(
    ...
    fairness = ifelse(
      str_detect(targetingWhy, "specific individual conduct") | appealJudgment == "yes", 1, 0) + 
        ifelse(hearingsPublic == "yes", 1, 0) + 
        ifelse(courtChallenge != "no court challenge" & courtChallenge != "don't know" & !is.na(courtChallenge), 
      1, 0
    )
  ) 
cat("done.\n\n") 
...
db[["ICCaccused"]] <- db[["Accused"]] |> 
  ...
  select(
    trialID, accusedID, country, ccode_target, name, position_desc, position_desc_fr, 
    ICC_arrest_warrant, ICC_arrestAppear, ICC_confirm_charges, ICC_proceedings, ICC_withdrawnDismissed
  ) |> 
...
not <- c("histname", "cid_who", "ldc", "lldc", "sids", "income_wb", "iso3c_wb", "region_wb2")
first <- c(
  "country", "country_case", "year", "ccode_cow", "ccode_ksg", "m49", "isoa3", 
  "country_id_vdem", "region", "subregion", "intregion", "region_wb", "micro_ksg"
)
...
df <- df |>
  left_join(
    db[["CountryYears"]] |>
      select(!any_of(c(
        "country", "cyID", "country_case", "ccode_case", "beg", "end", "country_label", 
        "country_label_fr", "region", "tjet_focus", "reg_democ", "reg_autoc", "reg_trans", "conflict"
      ))) |>
  ...
  mutate(
    ICC_arrest_warrant = ifelse(is.na(ICC_arrest_warrant), 0, ICC_arrest_warrant), 
  ...
  mutate(
    across(
      c(
        "uninv_beg", "uninv", "uninv_dompros_beg", "uninv_dompros", "uninv_evcoll_beg", 
...
not <- c(not, 
  "regime_sample", "reg_democ", "reg_autoc", "reg_trans", "transition", "conflict", 
  "sample_trans", "sample_confl_25", "sample_confl_100", "sample_confl_1000", "sample_combi"
) 
then <- names(df)[!names(df) %in% c(first, not)]
df <- df |> 
...
db[["Countries"]] <- db[["Countries"]] |>
  mutate(
    across(
      all_of(
        c(
          "auto_conflict_fr", "auto_regime_fr", "txt_amnesties_fr", "txt_conflict_fr", "txt_domestic_fr", 
          "txt_foreign_fr", "txt_intl_fr", "txt_intro_fr", "txt_regime_fr", "txt_reparations_fr", 
          "txt_summary_fr", "txt_tcs_fr", "txt_TJ", "txt_TJ_fr", "txt_un_fr", "txt_vetting_fr"
        )
      ), 
