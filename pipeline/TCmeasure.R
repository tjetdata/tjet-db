...
TCmeasure <- function(cy, new_col_name, 
  ...
  all_powers_vars <- c("investigateStateMembers", "compelTestimony", 
                       "grantAmnesty", "supportProsecutions", 
                       "namePerpetrators", "recommendInstitutionalReforms", 
                       "allocateReparations", "penalizeIndividuals", 
  ...
  if(missing(filter_nexus_vars) | sum(!filter_nexus_vars %in% all_nexus_vars) > 0) 
    stop("Missing argument: for subsetting, specify in 'filter_nexus_vars' one or more of:", 
         "\n  ", paste(all_nexus_vars, collapse = "; "))
  
  if(missing(filter_crimes_vars) | sum(!filter_crimes_vars %in% all_crimes_vars) > 0) 
    stop("Missing argument: for subsetting, specify in 'filter_crimes_vars' one or more of:",
  ...
  if(!is.null(harms_vars)) {
    ...
    ) stop("The 'harms_vars' can only be used with filter_crimes_vars == 'all'", 
           "  and other scale arguments set to NULL.") 
    error <- expression(
      stop("Incorrect argument: for a scale, specify in 'harms_vars' one or more of:",
  ...
  if(operated) {
    ...
    warning("TCs that met the criteria but never operated can only be based on \n",
