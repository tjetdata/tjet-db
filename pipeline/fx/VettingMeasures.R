vet_spells <- db[["Vettings"]] |>
  ...
  unnest(cols = c(year)) |>
  mutate(
    type_dismissal_created = ifelse(
      is.na(alterationOf) & year == yearStart,
      type_dismissal,
      0
    ),
    type_ban_created = ifelse(
      is.na(alterationOf) & year == yearStart,
      type_ban,
      0
    ),
    type_declass_created = ifelse(
      is.na(alterationOf) & year == yearStart,
      type_declass,
      0
    )
  ...
  select(
    ...
    fitsConflictTJ,
    type_dismissal,
    type_dismissal_created,
    type_ban,
    type_ban_created,
    type_declass,
    type_declass_created,
    ban_from_elected,
    conduct,
    public,
    fairness
