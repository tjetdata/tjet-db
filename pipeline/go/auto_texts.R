...
autoprep$amnesties <- amnesties |>
  left_join(countries |> select(ccode, ccode_case), by = "ccode") |>
  group_by(ccode_case) |>
  reframe(
...
autoprep$conflicts <- db[["ConflictDyads"]] |>
  mutate(
    internationalized = ifelse(str_detect(intensity, "Internationalized"), 1, 0)
  ) |>
  rowwise() |>
...
vars_dom <- c(
  ...
  "tran_trs_dom_dtj_ctj_sta",
  "regu_trs_dom_sta",
  "tran_cce_dom_dtj_ctj_sta",
  "regu_cce_dom_sta",
...
data[["Domestic_cy"]] <- db[["dl_tjet_cy"]] |>
  ...
  reframe(
    ...
    tran_trs_dom_dtj_ctj_sta_hi = sum(
      tran_trs_dom_dtj_ctj_sta_hi,
      na.rm = TRUE
    ),
    tran_cce_dom_dtj_ctj_sta_hi = sum(
      tran_cce_dom_dtj_ctj_sta_hi,
      na.rm = TRUE
    ),
...
data[["Vettings"]] <- db[["Vettings"]] |>
  left_join(
    countries |> select(country_case, ccode, ccode_case),
    by = c(ccode_cow = "ccode")
  ) |>
  mutate(
        ...
        c(
          if (amnesties > 0) {
            paste(
              ...
              ifelse(
                min(amnesties_yrs) == max(amnesties_yrs),
          ...
          if (domestic > 0) {
            paste(
              domestic,
              "domestic",
              ifelse(domestic == 1, "trial", "trials"),
              "starting",
              ifelse(
                min(domestic_yrs) == max(domestic_yrs),
          ...
          if (foreign > 0) {
            paste(
              foreign,
              "foreign",
              ifelse(foreign == 1, "trial", "trials"),
              "starting",
              ifelse(
                min(foreign_yrs) == max(foreign_yrs),
          ...
          if (intl > 0) {
            paste(
              intl,
              "international",
              ifelse(intl == 1, "trial", "trials"),
              "starting",
              ifelse(
                min(intl_yrs) == max(intl_yrs),
          ...
          if (reparations > 0) {
            paste(
              reparations,
              "reparations",
              ifelse(reparations == 1, "policy", "policies"),
              "created",
              ifelse(
                min(reparations_yrs) == max(reparations_yrs),
          ...
          if (tcs > 0) {
            paste(
              tcs,
              "truth",
              ifelse(tcs == 1, "commission", "commissions"),
              "mandated",
              ifelse(
                min(tcs_yrs) == max(tcs_yrs),
          ...
          if (vettings > 0) {
            paste(
              vettings,
              "vetting",
              ifelse(vettings == 1, "policy", "policies"),
              "starting",
              ifelse(
                min(vettings_yrs) == max(vettings_yrs),
...
autotxt[["Transitions"]] <- autoprep[["transitions"]] |>
  ...
  mutate(
    text = paste(
      "Based on well-known democracy data, TJET records ",
...
autotxt[["Conflicts"]] <- autoprep[["conflicts"]] |>
  ...
  mutate(
    text = "",
    text = paste(
      "Based on the Uppsala Conflict Data Program, TJET records ",
      episodes,
      ...
      ifelse(
        length(years) > 1 & length(years) < max(years) - min(years) + 1,
        paste(" (during ", length(years), " calendar years)", sep = ""),
        ""
      ),
      ", involving ",
      dyads,
...
autoprep[["rankings"]] |>
  ...
  mutate(
    legacy = paste(
      ...
      "out of",
      n,
      "on TJET's legacy of violence index"
    ),
  ...
  mutate(
    text = list(
      paste(
        ...
        ifelse(beg == end, paste("in", beg), paste("between", beg, "and", end)),
        ".",
        sep = ""
      ) |>
        n_transform()
...
autotxt[["Domestic_cy"]] <- data[["Domestic_cy"]] |>
  ...
  mutate(
    ...
    text = list(c(
      text,
      if (
        tran_trs_dom_dtj_sta > 0 |
          regu_trs_dom_sta > 0 |
          tran_trs_dom_ctj_sta > 0 |
          tran_trs_dom_ctj_opp > 0 |
            ...
            c(
              ...
              if (regu_trs_dom_sta > 0) {
                paste(
                  ...
                  ifelse(regu_trs_dom_sta == 1, "prosecution", "prosecutions"),
                  "of state agents, in which",
                  regu_cce_dom_sta,
                  ifelse(regu_cce_dom_sta == 1, "person was", "persons were"),
                  "convicted"
                )
              },
    ...
    text = list(c(
      ...
      if (tran_trs_dom_dtj_ctj_sta_hi > 0) {
        paste(
          "In",
          tran_trs_dom_dtj_ctj_sta_hi,
          ifelse(tran_trs_dom_dtj_ctj_sta_hi == 1, "trial", "trials"),
...
autotxt[["Intl"]] <- data[["Intl_cy"]] |>
  ...
  mutate(
    text = paste(
      ...
      trs_int_hrs_con_all,
      " international ",
      ifelse(trs_int_hrs_con_all == 1, "prosecution ", "prosecutions "),
      # " starting ",
      ifelse(
        beg == end,
        paste("in", beg),
      ...
      ", which led to ",
      cce_int_hrs_con_all,
      ifelse(cce_int_hrs_con_all == 1, " conviction. ", " convictions. "),
      sep = ""
    ) |>
      n_transform(),
  ...
  mutate(
    ...
    text = list(c(
      ...
      if (!is.na(ICC_investigation)) {
        paste(
          "The first investigation of a specific case began in ",
          ICC_investigation,
          ".",
          sep = ""
        )
...
autotxt[["ICCaccused"]] <- data[["ICC-accused"]] |>
  ...
  mutate(
    ...
    text = list(c(
      ...
      if (count_proceed > 0) {
        paste(
          "Proceedings began in ",
          count_proceed,
          ifelse(count_proceed == 1, " case ", " cases "),
          ifelse(
            length(unique(ICC_proceedings)) == 1,
...
autotxt[["UNinvestigations"]] <- data[["Investigations"]] |>
  mutate(
    country = ifelse(
      country == "Iran/Iraq" & ccode_case == 630,
      "Iran",
      country
    ),
    country = ifelse(
      country == "Iran/Iraq" & ccode_case == 645,
      "Iraq",
      country
    )
  ) |>
  ...
  mutate(
    beg = ifelse(
      ccode_case == 437 &
        mandate == "Office of the UN High Commissioner for Human Rights",
      beg,
      min(beg)
    ),
    end = ifelse(
  ...
  distinct() |>
  mutate(
    country = ifelse(country == "Yugoslavia", "Serbia and Montenegro", country)
  ) |>
  rowwise() |>
  mutate(
    text = paste(
...
autotxt[["Reparations"]] <- data[["Reparations"]] |>
  mutate(
    implemented = ifelse(
      !is.na(yearBegin) | !is.na(individualsRepairedEstimate),
      1,
      0
    ),
    ...
    beneficiariesCount = ifelse(
      is.na(beneficiariesCount),
      0,
      beneficiariesCount
    ),
    individualsRepairedEstimate = ifelse(
      is.na(individualsRepairedEstimate),
      0,
      individualsRepairedEstimate
    )
  ) |>
...
autotxt[["TruthCommissions"]] <- data[["TruthCommissions"]] |>
  mutate(
    ...
    yearBeginOperation = ifelse(
      is.na(yearBeginOperation) & !is.na(yearCompleteOperation),
      yearPassed,
      yearBeginOperation
    )
  ) |>
  ...
  mutate(
    ...
    yearCompleteOperation = list(yearCompleteOperation[
      !is.na(yearCompleteOperation)
    ]),
    text = paste(
      country_case,
      " mandated ",
    ...
    text = list(c(
      text,
      if (
        length(yearBeginOperation) == count &
          length(yearCompleteOperation) == 0 &
          count == 1
      ) {
        paste(
          "The commission began its operations in ",
    ...
    text = list(c(
      ...
      if (
        length(yearBeginOperation) == count &
          length(yearCompleteOperation) != 0 &
          length(yearCompleteOperation) < count
      ) {
        paste(
          length(yearCompleteOperation),
          " completed ",
    ...
    text = list(c(
      ...
      if (
        length(yearBeginOperation) < count &
          length(yearCompleteOperation) == length(yearBeginOperation)
      ) {
        paste(
    ...
    text = list(c(
      text,
      if (
        length(yearBeginOperation) < count & length(yearCompleteOperation) == 0
      ) {
        paste(
          length(yearBeginOperation),
    ...
    text = list(c(
      ...
      if (finalReportIssued > 0) {
        paste(
          ...
          ifelse(
            reportPubliclyAvailable > 0,
            paste(
              ifelse(
                reportPubliclyAvailable < finalReportIssued,
...
autotxt <- db[["Countries"]] |>
  ...
  mutate(
    summary = ifelse(
      is.na(summary),
      paste(
        "TJET has found no information on transitional justice in ",
        country_case,
        ".",
        sep = ""
      ),
...
chk <- db[["Countries"]] |>
  filter(include) |>
  full_join(autotxt, by = c("country_case", "ccode_case")) |>
  mutate(
    chk_summary = ifelse(
      auto_summary != summary | (!is.na(summary) & is.na(auto_summary)),
      1,
      0
    ),
    chk_regime = ifelse(
      auto_regime != regime | (!is.na(regime) & is.na(auto_regime)),
      1,
      0
    ),
    chk_conflict = ifelse(
      auto_conflict != conflict | (!is.na(conflict) & is.na(auto_conflict)),
      1,
      0
    ),
    chk_amnesties = ifelse(
      auto_amnesties != amnesties | (!is.na(amnesties) & is.na(auto_amnesties)),
      1,
      0
    ),
    chk_domestic = ifelse(
      auto_domestic != domestic | (!is.na(domestic) & is.na(auto_domestic)),
      1,
      0
    ),
    chk_foreign = ifelse(
      auto_foreign != foreign | (!is.na(foreign) & is.na(auto_foreign)),
      1,
      0
    ),
    chk_intl = ifelse(
      auto_intl != intl | (!is.na(intl) & is.na(auto_intl)),
      1,
      0
    ),
    chk_reparations = ifelse(
      auto_reparations != reparations |
        (!is.na(reparations) & is.na(auto_reparations)),
      1,
      0
    ),
    chk_tcs = ifelse(
      auto_tcs != tcs | (!is.na(tcs) & is.na(auto_tcs)),
      1,
      0
    ),
    chk_vetting = ifelse(
      auto_vetting != vetting | (!is.na(vetting) & is.na(auto_vetting)),
      1,
      0
    ),
    chk_un = ifelse(
      auto_un != un | (!is.na(un) & is.na(auto_un)),
      1,
      0
    ),
  ) |>
...
if (nrow(chk) > 0) {
  warning("Some auto-texts in Airtable do not match the current data!")
}
