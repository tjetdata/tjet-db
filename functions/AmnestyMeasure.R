AmnestyMeasure <- function(cy, 
  ...
  nexus <- c(all = "all", dtj = "fitsPostAutocraticTJ", 
  ...
  error <- expression(
    stop("Invalid argument for 'peace_vars', select one or more of:", 
  ...
  var_name <- str_flatten(c("amnesty", nexus_vars, settlement, 
                            who_opts, what_opts), collapse = "_") %>% 
