...
# problems(read_csv(here("conflicts/original_data/ucdp-prio-acd-251.csv")))
read_csv(here("conflicts/original_data/ucdp-prio-acd-251.csv")) %>% 
  filter(type_of_conflict > 2) %>% 
  group_by(conflict_id) %>%
  mutate(year_beg = min(year),
         year_end = max(year),
         ep_end_date = if_else(is.na(ep_end_date) & year == 2024 & ep_end == 0, 
                               as_date("2025-01-01"), ep_end_date),
         date_end = max(ep_end_date, na.rm = TRUE)) %>%
  ungroup() %>%
  mutate(date_end = if_else(date_end == as_date("2025-01-01"), 
                            NA, date_end)) %>%
  select(conflict_id, location, gwno_loc, year_beg, start_date, year_end, date_end, 
...
# problems(read_csv(here("conflicts/original_data/ucdp-dyadic-251.csv")))
## not using these columns
read_csv(here("conflicts/original_data/ucdp-dyadic-251.csv")) %>% 
  filter(type_of_conflict > 2) %>%
  group_by(dyad_id) %>%
  mutate(year_beg = min(year),
         year_end = max(year)) %>%
